...
def rest_get(table, params=None):
    ...
    if resp.status_code in (416, 404): return []
...
for batch_idx in range((len(all_symbols) + batch_size - 1) // batch_size):
    ...
    if all_rows:
        ...
        print(f'Batch {batch_idx + 1}: inserted {len(all_rows)} rows (total: {total_inserted})')

print(f'BACKFILL_COMPLETE: {total_inserted} total rows inserted')
